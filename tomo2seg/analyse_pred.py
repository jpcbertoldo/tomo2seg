...
from typing import Optional, ClassVar, Type, Tuple, List, Callable, Dict
    ...
    class Compute:
        ...
        def assert_dependency(self, master: str, dependent: str):
            ...
            if dependent_value == True:
                assert master_value == True, f"{dependent=}={dependent_value} but {master=}={master_value}"
    ...
    def __post_init__(self):
        ...
        for field in needs_to_be_computed_if_saved:
            if getattr(self.save, field) == True:
                assert getattr(self.compute, field) == True, f"{field=} must be computed to be saved: {getattr(self.save, field)=} but {getattr(self.compute, field)=}"
...
class AnalysePredOuputs(BaseOutputs):
    ...
    def layers_correlation(self, axis: int, label: Optional[int]) -> Path:
        ...
        return self.layers_correlation_dir / f"layers-correlation.label={label}.{axis=}.npy"
...
def report2table(report_dict: dict, labels_names: List[str]) -> Tuple[DataFrame, DataFrame, DataFrame]:
