...
class Type(Enum):
    ...
    def input_is_3d(self) -> bool:
        ...
        if self in (self.d2half, self.d3,):
            return True

        elif self in (self.d2,):
            return False
    ...
    def output_is_3d(self) -> bool:
        ...
        elif self in (self.d2, self.d2half,):
            return False
...
class Model:
    ...
    def build_from_model_name(cls, name: str):
        ...
        return cls(master_name, version, fold, runid)

    def __post_init__(self):
        if self.factory_function is not None and callable(self.factory_function):
            self.factory_function = (
                f"{self.factory_function.__module__}.{self.factory_function.__name__}"
            )
    ...
    @property
    def name(self) -> str:
        return Model.vars2name(self.master_name, self.version, self.fold, self.runid,)
    ...
    def autosaved2_model_path(self) -> Path:
        return (
            self.model_path
            / f"{self.name}.autosaved.{{epoch:03d}}-{{val_loss:.6f}}.hdf5"
        )
    ...
    def autosaved2_all(self, with_loss=False) -> Optional[List[Path]]:
        ...
        is_autosaved_model = re.compile(
            f"{self.name}.autosaved.\d{{3,}}-(0.\d{{6,}}).hdf5".replace(".", "\.")
        )
        ...
        return [
            (self.model_path / m[0], m[1],) if with_loss else self.model_path / m[0]
...
def estimate_max_batch_size_per_gpu(
    model: KerasModel, max_internal_voxels: int, crop_shape: Tuple[int]
) -> int:

    model_internal_nvoxel_factor = utils.get_model_internal_nvoxel_factor(model)

    max_batch_nvoxels = int(
        np.floor(max_internal_voxels / model_internal_nvoxel_factor)
    )

    crop_nvoxels = functools.reduce(operator.mul, crop_shape)

    return max(1, int(np.floor(max_batch_nvoxels / crop_nvoxels)))
