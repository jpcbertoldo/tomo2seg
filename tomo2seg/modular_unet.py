...
def u_net(
    ...
    """
    unet_block_kwargs = {**unet_block_kwargs, **dict(convlayer=convlayer)}

    unet_block = functools.partial(generic_unet_block, **unet_block_kwargs)

    unet_down_kwargs = {
        **unet_down_kwargs,
        **dict(conv_sampling=updown_conv_sampling, convlayer=convlayer,),
    }
...
kwargs_vanilla00 = dict(
    ...
    unet_block_kwargs=dict(kernel_size=3, res=False, batch_norm=False, dropout=0,),
    unet_down_kwargs={},
    unet_up_kwargs={},
)
...
kwargs_vanilla01 = dict(
    ...
    sigma_noise=0,
    updown_conv_sampling=False,
    unet_block_kwargs=dict(kernel_size=3, res=True, batch_norm=False, dropout=0,),
    unet_down_kwargs={},
    unet_up_kwargs={},
)
...
kwargs_vanilla02 = dict(
    ...
    sigma_noise=0,
    updown_conv_sampling=False,
    unet_block_kwargs=dict(kernel_size=3, res=True, batch_norm=True, dropout=0,),
    unet_down_kwargs={},
    unet_up_kwargs={},
)
...
kwargs_vanilla03 = dict(
    ...
    unet_block_kwargs=dict(kernel_size=3, res=True, batch_norm=True, dropout=0,),
    unet_down_kwargs=dict(batchnorm=True),
    unet_up_kwargs=dict(batchnorm=True),
)
...
def u_net2halfd_IIencdec(
    ...
    x0_splitted = [
        layers.Lambda(lambda x_: tf_slice(x_, (0, 0, 0, ch), (-1, -1, -1, 1)))(x0)
    ...
    for i in range(depth):
        ...
        block = unet_block(
            name=block_name, nb_filters_1=nb_filters_begin, nb_filters_2=nb_filters_end
        )
        ...
        for layer_idx in range(nlayers):
            ...
            y, block_reused_layers = block(
                xs[layer_idx], reused_layers=block_reused_layers
            )
            xs[layer_idx] = y

            skips[(i, layer_idx)] = y

            y, down_reused_layers = down_block(
                xs[layer_idx], reused_layers=down_reused_layers
            )
            xs[layer_idx] = y
    ...
    for i in reversed(range(depth)):
        ...
        up_block = unet_up(nb_filters=nb_filters_up, name=block_name,)
        block = unet_block(
            name=block_name, nb_filters_1=nb_filters_conv, nb_filters_2=nb_filters_conv,
        )
        ...
        for layer_idx in range(nlayers):
            ...
            y, up_reused_layers = up_block(
                xs[layer_idx], reused_layers=up_reused_layers
            )
            xs[layer_idx] = y

            x_skip = skips[(i, layer_idx)]

            xs[layer_idx] = layers.concatenate(
                [x_skip, xs[layer_idx]], axis=-1, name=f"concat_{i}-layer_{layer_idx}"
            )
...
kwargs_IIencdec03_debug = {
    ...
    **dict(
        depth=2,
        updown_conv_sampling=False,
        unet_block_kwargs=dict(kernel_size=3, dropout=0, batch_norm=False, res=False,),
    ),
}
