...
#     x = x0 = layers.Input(input_shape, name="input")
    ...
    for i in reversed(range(depth)):
        ...
        x, _ = up_block(x, reused_layers=None)
        
        xs_skip = [skips[(i, layer_idx)] for layer_idx in range(nlayers)]
        
        layer_name = f"concat-{i}"
        x = layers.concatenate(xs_skip + [x], axis=-1, name=layer_name)
        
        block_name = f"dec-block-{i}"
        block = unet_block(name=block_name, nb_filters_1=nb_filters_conv, nb_filters_2=nb_filters_conv)
